...
import com.meteocontrol.client.models.SensorDetail;

import java.io.IOException;

public class Sensor extends SubEndpoint {

    public Sensor(EndpointInterface parent) {
        this.url = "";
        this.api = parent.getApiClient();
        this.parent = parent;
    }

    public SensorDetail get() throws IOException {
        String systemJson = this.api.run(this.getUrl());
        ObjectMapper mapper = new ObjectMapper();
        JsonNode rootNode = mapper.readValue(systemJson, JsonNode.class);
        return mapper.readValue(rootNode.get("data").toString(), SensorDetail.class);
    }
