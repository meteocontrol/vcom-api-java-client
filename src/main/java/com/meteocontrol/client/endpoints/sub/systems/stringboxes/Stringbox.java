...
import com.meteocontrol.client.models.StringboxDetail;

import java.io.IOException;

public class Stringbox extends SubEndpoint {

    public Stringbox(EndpointInterface parent) {
        this.url = "";
        this.api = parent.getApiClient();
        this.parent = parent;
    }

    public StringboxDetail get() throws IOException {
        String systemJson = this.api.run(this.getUrl());
        ObjectMapper mapper = new ObjectMapper();
        JsonNode rootNode = mapper.readValue(systemJson, JsonNode.class);
        return mapper.readValue(rootNode.get("data").toString(), StringboxDetail.class);
    }
